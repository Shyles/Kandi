...
Pieni maailma -verkkojen ominaisuuksia voidaan kuvailla Wattzin ja Strogatzin \cite{CollectiveWattz} määrittelemillä suureilla \textit{keskimääräinen polun pituus} ja \textit{ryhmittymiskerroin}. Keskimääräinen polun pituus verkossa kuvaa nimensä mukaisesti keskiarvoa kaikkien solmujen välisistä lyhimmistä poluista. Ryhmittymiskerroin \( \mathcal{C} \) taas kuvaa kuinka hyvin lähekkäiset solmut ovat yhdistyneet toisiinsa. Esimerkiksi sosiaalisissa verkoissa suure vastaa kysymykseen, kuinka moni ystävistäni on ystävyksiä keskenään.

\begin{maaritelma}[Ryhmittymisekerroin]
Olkoon \(\mathcal{C} \) verkon \(G\) ryhmittymiskerroin, \(v\) suuntamattoman verkon \(G\) solmu ja solmulla \(v\) muuttujan \(k_v\) määrä naapureita. Tällöin solmun \(v\) ja sen naapurien välillä voi olla enintään \(k_v(k_v - 1)/2\) kaarta. Olkoon \(\mathcal{C}_v\) verkossa \(G\) oleva määrä kyseisistä sallituista kaarista. Määritämme ryhmittymiskertoimen \(\mathcal{C}\) muuttujan \(\mathcal{C}_v\) keskiarvoksi yli kaikkien solmujen \(v\) verkossa \(G\).
\end{maaritelma}
...
jollain vakiolla 
\( p \geq 1\ \). 
Solmulla \( u \) on myös vakiomäärä, \( q \geq 0 \), \textit{etäkontakteja}. Etäkontaktit solmujen \(u\) ja \(v\) välille muodostetaan satunnaisesti todennäköisyysfunktiolla, joka riippuu vakiosta \( r \geq 0 \) ja etäisyydestä \( \distance{u}{v} \). Tarkemmin, etäkontakti muodostetaan satunnaiskokeella solmusta \( u \) solmuun \( v \) todennäköisyydellä
\[
\frac{
	\left[
		\distance{u}{v}
...
Lauseet \ref{Logaritmisesti nopein} ja \ref{Hitaampi kun ei 2}
kertovat odotetun toimitusajan olevan enimmillään logaritmisesti riippuva vakiosta \( n\), jos ja vain jos \( r = 2\). Algoritmi \( \mathcal{A} \) pyrkii aina lähettämään viestiä mahdollisimman lähelle kohdetta \( y\). Mikäli etäkontaktit eivät riipu tarpeeksi etäisyydestä, \(r  < 2\), niin viestin seuraavalla vastaanottajalla on huonompi todennäköisyys olla etäkontakti kohteen lähellä. Tähän myös perustuu Lauseen \ref{Ei voi löytää} todistus. Mikäli etäkontaktit ovat muodostettu täysin sattumanvaraisesti, algoritmi ei voi luotettavasti löytää solmuja joilla on etäkontakti entistä lähempänä kohdetta. Jos etäkontaktit riippuvat liikaa etäisyydestä \( r > 2 \), algoritmin nopeus lähentyä kohdetta pienenee etäkontaktien ollessa lähempänä viestiä lähettävää solmua.
